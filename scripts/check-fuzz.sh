...
multiplier="${1:-1}"
budgets="${FUZZ_BUDGETS:-verification/fuzz-budgets.tsv}"
...
while IFS="$tab" read -r package target iterations extra; do
	...
	test "$iterations" -ge 1 && test "$iterations" -le 10000000 || {
		printf 'fuzz iterations on row %d must be between 1 and 10000000\n' "$line" >&2
		exit 1
	}
	runs=$((iterations * multiplier))
	if test -f "${package}/go.mod"; then
		(
			cd "${package}"
			go test . -run '^$' -fuzz "^${target}$" -fuzztime "${runs}x"
		)
	else
		go test "$package" -run '^$' -fuzz "^${target}$" -fuzztime "${runs}x"
	fi
	targets=$((targets + 1))
done < "$budgets"
