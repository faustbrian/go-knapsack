...
func TestEvidenceSourceFilesExcludeIgnoredArtifacts(t *testing.T) {
	t.Parallel()

	repository := t.TempDir()
	if err := os.MkdirAll(filepath.Join(repository, "vendor"), 0o755); err != nil {
		t.Fatal(err)
	}
	for path, contents := range map[string]string{
		...
		"untracked.json":   "{}\n",
		"vendor/hidden.go": "package hidden\n",
	} {
	...
	if output, err := exec.Command("git", "-C", repository, "add", "tracked.go", ".gitignore").CombinedOutput(); err != nil {
		t.Fatalf("index evidence fixture: %v\n%s", err, output)
	}

	files := evidenceSourceFilesIn(t, repository)
	if !slices.Equal(files, []string{"tracked.go", "untracked.json"}) {
		t.Fatalf("evidence source files = %v", files)
	}
}
...
func validateWorkflowEvidence(t *testing.T, proof workflowProof, knownTests map[string]bool) {
	...
	if proof.LintCommand != "golib repository check" ||
		proof.ToolingRelease != "github.com/faustbrian/go-library-tools v1.4.0" ||
...
func evidenceSourceFilesIn(t *testing.T, directory string) []string {
	...
	command := exec.Command("git", "ls-files", "-co", "--exclude-standard", "--", ".")
