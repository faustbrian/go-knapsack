...
func TestTotalsAndOrderingMatchDirectExactMoneyArithmetic(t *testing.T) {
	...
	for iteration := range 100 {
		entries := make([]moneyobjective.Entry, 4)
		...
		for index := range entries {
			...
			entries[index] = moneyobjective.Entry{TypeID: typeID, Cost: cost}
		...
		forwardCosts, err := moneyobjective.NewWithPolicy(forward, policy)
		if err != nil {
			t.Fatal(err)
		}
		reverseCosts, err := moneyobjective.NewWithPolicy(reverse, policy)
		if err != nil {
			t.Fatal(err)
		}
...
func TestCostsAreSafeForConcurrentSolverUse(t *testing.T) {
	...
	costs, err := moneyobjective.New(map[string]money.Money{"small": one, "large": large})
	if err != nil {
		t.Fatal(err)
	}
