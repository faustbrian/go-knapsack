...
func FuzzCostTypeIdentifiers(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, typeID string) {
		...
		costs, newErr := moneyobjective.NewWithLimits(
			map[string]money.Money{typeID: cost},
			moneyobjective.Limits{MaxTypes: 1, MaxIDBytes: 32},
		)
		valid := strings.TrimSpace(typeID) != "" && len(typeID) <= 32
		if !valid {
			if !errors.Is(newErr, moneyobjective.ErrInvalidCosts) {
...
func FuzzCostMappings(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, leftID, rightID string, amount int64, scale uint8, maxIDBytes uint16, allowNegative bool) {
		...
		policy := moneyobjective.Policy{
			Limits:             moneyobjective.Limits{MaxTypes: 2, MaxIDBytes: uint32(maxIDBytes%65 + 1)},
			AllowNegativeCosts: allowNegative,
		}
		entries := []moneyobjective.Entry{{TypeID: leftID, Cost: value}, {TypeID: rightID, Cost: value}}
		first, firstErr := moneyobjective.NewFromEntries(entries, policy)
		second, secondErr := moneyobjective.NewFromEntries(entries, policy)
		if (firstErr == nil) != (secondErr == nil) {
			t.Fatalf("construction changed outcome: %v then %v", firstErr, secondErr)
		}
		if firstErr != nil {
			if !errors.Is(firstErr, moneyobjective.ErrInvalidCosts) || !errors.Is(secondErr, moneyobjective.ErrInvalidCosts) {
...
func FuzzSolverCostObjective(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, smallAmount, largeAmount int64, rawMaxTypes uint8, reverse, cancelled bool) {
		...
		costs, newErr := moneyobjective.NewWithPolicy(mapping, moneyobjective.Policy{
			Limits:             moneyobjective.Limits{MaxTypes: maximumTypes, MaxIDBytes: 5},
