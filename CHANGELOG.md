...
- Include adapter subprocess diagnostics when nested interoperability checks
  fail.
- Validate committed platform-specific benchmark evidence against its recorded
  environment instead of the machine running repository checks.
- Resolve the internal comparison harness against the current monorepo checkout
  instead of an unpublished Knapsack archive.
...
- Enforce the nested Go Money objective fuzzer through the canonical fuzz
  budget instead of merely inventorying it as test source.
